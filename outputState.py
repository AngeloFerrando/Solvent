...
def constructor(xa1, xn1, awNow, awNext, wNow, wNext, t_aw, t_w, block_num, bNow, bNext, t_b):
	...
	And(And(
	True,
	next_state_tx(awNow, awNext, wNow, wNext, t_b[0], bNext)), True, next_state_tx(awNow, awNext, wNow, wNext, t_b[0], bNext)))
...
def unlock(xa1, xn1, awNow, awNext, wNow, wNext, t_aw, t_w, block_num, bNow, bNext, t_b):
	...
	Not(Not(bNow)), 
		...
		And(t_b[0] == True,True, next_state_tx(awNow, awNext, wNow, wNext, t_b[0], bNext))))))
...
def pay(xa1, xn1, pay_amount, awNow, awNext, wNow, wNext, t_aw, t_w, block_num, bNow, bNext, t_b):
	...
	If(
	Not(pay_amount >= 0), 
		next_state_tx(awNow, awNext, wNow, wNext, bNow, bNext), And(send(xa1, pay_amount, wNow, t_w[0], awNow, t_aw[0]), True, next_state_tx(t_aw[0], awNext, t_w[0], wNext, t_b[0], bNext)))),True, next_state_tx(t_aw[0], awNext, t_w[0], wNext, t_b[0], bNext))))))
