...
def constructor(xa1, xn1, constructor_o,constructor_d, awNow, awNext, wNow, wNext, t_aw, t_w, block_num, oracleNow, oracleNext, t_oracle, deadlineNow, deadlineNext, t_deadline, par1Now, par1Next, t_par1, par2Now, par2Next, t_par2):
	...
	And(
	True,
	True)),True, next_state_tx(awNow, awNext, t_w[0], wNext, t_oracle[0], oracleNext, t_deadline[0], deadlineNext, t_par1[0], par1Next, par2Now, par2Next))))))
...
def join(xa1, xn1, awNow, awNext, wNow, wNext, t_aw, t_w, block_num, oracleNow, oracleNext, t_oracle, deadlineNow, deadlineNext, t_deadline, par1Now, par1Next, t_par1, par2Now, par2Next, t_par2):
	...
	Not(xa1!=par1Now), 
		next_state_tx(awNow, awNext, wNow, wNext, oracleNow, oracleNext, deadlineNow, deadlineNext, par1Now, par1Next, par2Now, par2Next), And(
		And(t_par2[0] == xa1,True, next_state_tx(awNow, awNext, t_w[0], wNext, oracleNow, oracleNext, deadlineNow, deadlineNext, par1Now, par1Next, t_par2[0], par2Next)))),True, next_state_tx(awNow, awNext, t_w[0], wNext, oracleNow, oracleNext, deadlineNow, deadlineNext, par1Now, par1Next, t_par2[0], par2Next)))),True, next_state_tx(awNow, awNext, t_w[0], wNext, oracleNow, oracleNext, deadlineNow, deadlineNext, par1Now, par1Next, t_par2[0], par2Next))))))
...
def win(xa1, xn1, win_winner, awNow, awNext, wNow, wNext, t_aw, t_w, block_num, oracleNow, oracleNext, t_oracle, deadlineNow, deadlineNext, t_deadline, par1Now, par1Next, t_par1, par2Now, par2Next, t_par2):
	...
	Not(Or(win_winner==par1Now,win_winner==par2Now)), 
		...
		And(If(
	Not(wNow >= 0), 
		next_state_tx(awNow, awNext, wNow, wNext, oracleNow, oracleNext, deadlineNow, deadlineNext, par1Now, par1Next, par2Now, par2Next), And(send(win_winner, wNow, wNow, t_w[0], awNow, t_aw[0]), True, next_state_tx(t_aw[0], awNext, t_w[0], wNext, oracleNow, oracleNext, deadlineNow, deadlineNext, par1Now, par1Next, par2Now, par2Next))),True, next_state_tx(t_aw[0], awNext, t_w[0], wNext, oracleNow, oracleNext, deadlineNow, deadlineNext, par1Now, par1Next, par2Now, par2Next)))),True, next_state_tx(t_aw[0], awNext, t_w[0], wNext, oracleNow, oracleNext, deadlineNow, deadlineNext, par1Now, par1Next, par2Now, par2Next)))),True, next_state_tx(t_aw[0], awNext, t_w[0], wNext, oracleNow, oracleNext, deadlineNow, deadlineNext, par1Now, par1Next, par2Now, par2Next)))),True, next_state_tx(t_aw[0], awNext, t_w[0], wNext, oracleNow, oracleNext, deadlineNow, deadlineNext, par1Now, par1Next, par2Now, par2Next))))))
...
def timeout(xa1, xn1, awNow, awNext, wNow, wNext, t_aw, t_w, block_num, oracleNow, oracleNext, t_oracle, deadlineNow, deadlineNext, t_deadline, par1Now, par1Next, t_par1, par2Now, par2Next, t_par2):
	...
	Not(1 >= 0), 
		next_state_tx(awNow, awNext, wNow, wNext, oracleNow, oracleNext, deadlineNow, deadlineNext, par1Now, par1Next, par2Now, par2Next), And(send(par2Now, 1, t_w[0], t_w[1], t_aw[0], t_aw[1]), True, next_state_tx(t_aw[1], awNext, t_w[1], wNext, oracleNow, oracleNext, deadlineNow, deadlineNext, par1Now, par1Next, par2Now, par2Next))),True, next_state_tx(t_aw[1], awNext, t_w[1], wNext, oracleNow, oracleNext, deadlineNow, deadlineNext, par1Now, par1Next, par2Now, par2Next)))),True, next_state_tx(t_aw[1], awNext, t_w[1], wNext, oracleNow, oracleNext, deadlineNow, deadlineNext, par1Now, par1Next, par2Now, par2Next)))),True, next_state_tx(t_aw[1], awNext, t_w[1], wNext, oracleNow, oracleNext, deadlineNow, deadlineNext, par1Now, par1Next, par2Now, par2Next))))))
...
new_state = And(And(xa[0] >= 0, xa[0] <= A, xn[0] >= 0),
               And([aw[0][j] >= 0 for j in range(A+1)]),
                  constructor(xa[0], xn[0], constructor_o[0],constructor_d[0],  aw[0], aw[1], w[0], w[1], t_aw[0], t_w[0], block_num[0], oracle[0], oracle[1], t_oracle[0], deadline[0], deadline[1], t_deadline[0], par1[0], par1[1], t_par1[0], par2[0], par2[1], t_par2[0]))
s.add(new_state)
for i in range(1, N):
...
for prop in {'liq1'}:
    for i, q in enumerate(queries[prop]):
        for j in range(0, len(q)):
            ...
            s2.add(qj)
            text= s2.to_smt2()
            filename = 'out/%s/statebased/%s/output_%s.smt2'%(prop, i, j)
            ...
            if not os.path.exists('out/%s/statebased/%s/'%(prop, i)):
                os.makedirs('out/%s/statebased/%s/'%(prop, i))
