...
print("Solving time: " + str(timeTot) + "s")
                        ...
                        for tk1 in range(0, self.__Tokens):
                            if 'ag{ag}{tk}(i+1)=='.format(ag=ag1, tk=tk1) not in aux:
                                if aux != '':
                                    ...
                                    aux = 'ag{ag}{tk}(i+1)==ag{ag}{tk}(i)'.format(ag=ag1, tk=tk1)
                    for tk1 in range(0, self.__Tokens):
    ...
    def visitProcedureExpr(self, ctx:TxScriptParser.ProcedureExprContext):
        ...
        preconditions = self.visit(ctx.requires)
        body = self.visit(ctx.body)
        if preconditions:
            return (start, end, preconditions + ', ' + body) # TBC
        else:
            return (start, end, body) # TBC
        # TBD
    ...
    def visitRequiresExpr(self, ctx:TxScriptParser.RequiresExprContext):
        equation = None
        for precondition in ctx.requireExpr():
            if equation:
    ...
    def visitMap(self, ctx:TxScriptParser.MapContext):
        ...
        return self.__mapInput[ctx.child.text] + '(i)' + '(' + arg + ')'
    ...
    def visitWithdraw(self, ctx:TxScriptParser.WithdrawContext):
        amount = self.visit(ctx.variable)
        agent = self.visit(ctx.agent)
        if 'ag' in agent:
            return 'And(' + 'tk{tk}(i)>=' + amount + ', ' + 'ag{ag}{tk}(i+1)==ag{ag}{tk}(i) + ' + amount + ', ' + 'tk{tk}(i+1)==tk{tk}(i) -' + amount + ')'
        else:
    ...
    def visitDeposit(self, ctx:TxScriptParser.DepositContext):
        ...
        agent = self.visit(ctx.agent)
        if 'sender' in agent:
            return 'And(' + 'ag{ag}{tk}(i)>=' + amount + ', ' + 'ag{ag}{tk}(i+1)==ag{ag}{tk}(i) - ' + amount + ', ' + 'tk{tk}(i+1)==tk{tk}(i) +' + amount + ')'
        else:
            raise Exception('Not supported right now')
    ...
    def visitBodyExprs(self, ctx:TxScriptParser.BodyExprsContext):
        equation = None
        for body in ctx.bodyExpr():
            if equation:
                equation = equation + ', ' + self.visit(body)
            else:
                equation = self.visit(body)
        return 'And(' + equation + ')'
...
# Visit a parse tree produced by TxScriptParser#firstOrderFormulasExpr.
    ...
    def visitGlobalMap(self, ctx:TxScriptParser.GlobalMapContext):
        domain, codomain = self.visit(ctx.mapDef)
        return ((domain, codomain), ctx.name.text, {})
    
     # Visit a parse tree produced by TxScriptParser#mapExpr.
    def visitMapExpr(self, ctx:TxScriptParser.MapExprContext):
        return ctx.domain.text, ctx.codomain.text
